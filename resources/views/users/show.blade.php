...
@section('content')
    ...
    <span>Nascimento: <span>{{ \Carbon\Carbon::parse($user->date_birth)->format('d/m/Y') . ' - ' . date('Y') - \Carbon\Carbon::parse($user->date_birth)->format('Y') . ' anos, ' . 12 - \Carbon\Carbon::parse($user->date_birth)->format('m') . ' meses e ' . date('d') . ' dias' }} </span><br>
